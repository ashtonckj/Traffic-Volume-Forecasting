...
#
#   Rscript tuning_lstm.R
#
#   HOW TO USE: edit GRID (section 3), run, read the leaderboard, fix the
#   parameters that clearly won, open up the next ones, run again. Configs
#   already in the CSV are skipped, so each pass only costs the new ones.
#
...
#
# FEATURES: the nine dataset columns as supplied. No cyclical (sine/cosine)
# encoding. This block MUST stay identical to lstm.R — if the two scripts build
# features differently, the tuning result does not describe the final model.
# =============================================================================
...
# =============================================================================
# 3. THE SEARCH SPACE  — edit this, run, then edit again.
#
# Suggested order of passes. Each pass fixes what the previous one settled and
# opens up the next parameter, so you never pay for a full factorial.
#
#   PASS 1 (below)  lookback {48, 168} x dropout {0, 0.1}
#                   Look-back matters more now than it did with the cyclical
#                   encoding: the network is no longer handed the daily and
#                   weekly cycle, so it has to infer periodicity from the
#                   window itself, and a 48-hour window contains no weekly
#                   cycle at all. Dropout rides along as a cheap check that
#                   "zero is best" still holds.
#
#   PASS 2          fix lookback + dropout at the pass-1 winner;
#                   open units {64, 128} and n_layers {1, 2}
#
#   PASS 3          fix those too; open learning_rate {3e-4, 1e-3, 3e-3}
#
# Add 336L to lookback only if 168 clearly beats 48 — it costs roughly twice
# as much as 168 and mostly repeats the weekly cycle a second time.
# =============================================================================
GRID <- list(
  lookback      = c(48L, 168L),
  units         = c(64L),
...
# =============================================================================
# 7. METRICS — all on the ORIGINAL traffic_volume scale
#
...
# =============================================================================
# 8. ONE FOLD — fresh weights every call, so nothing leaks between folds
# =============================================================================
...
for (i in seq_len(nrow(TODO))) {
  ...
  cat(sprintf("        MASE %.4f (sd %.4f)  MAE %.1f  RMSE %.1f  epochs %s  [%.0fs, ETA %.0f min]\n",
              row$mean_MASE, row$sd_MASE, row$mean_MAE, row$mean_RMSE,
              row$best_epochs, secs, eta_min))
}
