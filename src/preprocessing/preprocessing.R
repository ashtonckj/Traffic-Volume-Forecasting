...
# holiday and traffic_volume are identical within a timestamp; temp, rain,
# snow and clouds are averaged.
n_rows_before <- nrow(df)
df <- df %>%
  group_by(date_time) %>%
  summarise(
...
# ---- 4. Fix known mislabeled holiday date ----
...
# The raw data marks a holiday on its first hour only, so the label is
# propagated to every hour of that calendar date.
...
# ---- 6. Outlier detection figure (before cleaning) ----
# Four-panel diagnostic grid showing the physical anomalies that Section 7
# repairs, plus the raw traffic series for context.
